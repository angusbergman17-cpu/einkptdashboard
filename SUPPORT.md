# Supporting PTV-TRMNL

Thank you for considering supporting PTV-TRMNL! This project is developed and maintained by **Angus Bergman** as a passion project to make Australian public transport more accessible.

---

## 💝 Ways to Support

### Financial Support

| Platform | Link | Description |
|----------|------|-------------|
| **Buy Me a Coffee** | [buymeacoffee.com/angusbergman](https://buymeacoffee.com/angusbergman) | One-time donations |
| **GitHub Sponsors** | [github.com/sponsors/angusbergman17-cpu](https://github.com/sponsors/angusbergman17-cpu) | Monthly sponsorship |

### Non-Financial Support

- ⭐ **Star the repository** — helps visibility
- 🐛 **Report bugs** — [Open an issue](https://github.com/angusbergman17-cpu/einkptdashboard/issues)
- 💡 **Suggest features** — ideas welcome!
- 📖 **Improve documentation** — PRs appreciated
- 🗣️ **Spread the word** — tell friends, colleagues, Reddit, Twitter

---

## 🎯 What Your Support Enables

### Development
- 🚊 **More transit authorities** — expanding beyond Victoria to all Australian states
- 📱 **New device support** — TRMNL X, more Kindle models, custom hardware
- 🌏 **International expansion** — potential support for transit systems worldwide
- ⚡ **Performance improvements** — faster refresh, better reliability

### Infrastructure
- 🔧 **Testing devices** — hardware for development and testing
- 📊 **API costs** — Google Places, Mapbox for enhanced geocoding
- 🖥️ **Server costs** — hosting for documentation and demos

### Community
- 📖 **Documentation** — tutorials, guides, video content
- 🤝 **User support** — helping people get set up
- 🎨 **Design improvements** — better UI/UX

---

## 💰 Transparency

This is a **non-commercial, open-source project**. All donations go directly toward:

1. Development time and tools
2. Testing hardware (displays, devices)
3. API and hosting costs
4. Coffee ☕ (lots of coffee)

---

## 🙏 Thank You

Every contribution, whether financial or otherwise, helps make PTV-TRMNL better for everyone. 

If you're using PTV-TRMNL for your daily commute and finding it useful, even a small token of appreciation means the world.

**Built with ❤️ in Melbourne, Australia**

---

## 📧 Contact

- **Issues/Bugs**: [GitHub Issues](https://github.com/angusbergman17-cpu/einkptdashboard/issues)
- **Email**: Via the feedback form in the app
- **GitHub**: [@angusbergman17-cpu](https://github.com/angusbergman17-cpu)

---

<p align="center">
  <a href="https://buymeacoffee.com/angusbergman">
    <img src="https://img.shields.io/badge/Buy%20Me%20a%20Coffee-FFDD00?style=for-the-badge&logo=buy-me-a-coffee&logoColor=black" alt="Buy Me a Coffee">
  </a>
  &nbsp;
  <a href="https://github.com/sponsors/angusbergman17-cpu">
    <img src="https://img.shields.io/badge/GitHub%20Sponsors-EA4AAA?style=for-the-badge&logo=github-sponsors&logoColor=white" alt="GitHub Sponsors">
  </a>
</p>
