...
const documentationAudits = {
  ...
  async licenseHeaders() {
    ...
    const requiredPattern = /CC BY-NC|Creative Commons|Copyright.*Angus Bergman/i;
    const filesToCheck = [];
  ...
  async legalCompliance() {
    ...
    if (license) {
      ...
      if (license.includes('Creative Commons') || license.includes('CC BY-NC') || license.includes('BY-NC-4.0')) {
        result.passed = true;
        result.details.push('✓ CC BY-NC 4.0 license confirmed');
      }
